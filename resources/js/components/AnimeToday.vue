<template>
    <div className="broadcastTitles">
        <vue-glide>
            <vue-glide-slide v-for="title in today" :key="title.mal_id">
                <div className="box" data-id={index} key={index} style={{ width: 200 }}>
                    <figure className="item__title-box-image">
                        <a className="title-link" :href="`/ecma/titulos/${strToSlug(title.type)}/${this.strToSlug(title.title)}`">
...
export default {
  ...
  mounted () {
    ...
    fetch(`https://api.jikan.moe/v3/schedule/${getDay()}`)
      .then(res => res.json())
      .then(response => {
        this.today = response[getDay()];
      });
  },
  components: {
    [Glide.name]: Glide,
