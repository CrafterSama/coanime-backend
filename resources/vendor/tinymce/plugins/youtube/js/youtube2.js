 ...
 */
	...
	function get_video_iframe() {
		...
		var sHTML = '<div class="youtube"><embed title="' + $('#titleURL').val() + '" src="' + sEmbedUrl + '?wmode=opaque&modestbranding=1&theme=' + $('#skinURL').val() + '"></embed></div>';
		return sHTML;
	}
	function I_Insert() {
		var sEmbedUrl = secure + '://www.youtube.com/embed/' + getVideoId($('#inpURL').val());
        ...
        if (youtput == 'placeholder')
			...
			var sHTML = '<div class="youtube"><embed title="' + sTitle + '" src="' + sEmbedUrl + '?wmode=opaque&theme=' + $('#skinURL').val() + '"></embed></div>';
		I_InsertHTML(sHTML);
	}
